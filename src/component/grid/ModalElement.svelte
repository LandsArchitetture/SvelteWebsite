...
{#each related as rel, i}
    <div id={post.project.toString() + '.' + i} class="carousel-item relative">
        <img src="{getSrc(rel)}" loading="lazy" alt="{rel.project}" class="w-full object-cover" />
        <p class="absolute bottom-4 left-0 text-white text-xl font-bold p-2">{rel.text ? rel.text : ''}</p>
    </div>
